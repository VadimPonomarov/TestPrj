...
> Для Docker використовується `.env.docker` (входить до репозиторію). Для локального запуску використовується `.env.local` (входить до репозиторію; за потреби замініть значення під свій стенд).
