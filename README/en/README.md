...
- Clone once:
  ...
  ```
- All prerequisites (Python&nbsp;3.12, Poetry, Docker, Git) and deployment paths now live in:
  - `../uk/DEPLOYMENT.md` (UA)
  - `DEPLOYMENT.md` (EN)
  
  The guide covers:
  1. Recommended Docker-first flow via `deploy.py`.
  2. Classic `docker compose up` usage and db-only workflow for Scrapy.
  3. Local (non-Docker) setup through `deploy.local.py` or manual `poetry` commands.
  4. Troubleshooting tips, health checks and port requirements.
- Keep this README for architecture/API context; switch to `DEPLOYMENT.md` whenever you need environment-specific instructions.
...
| `nginx` | `80`   | Optional reverse proxy exposing static/media and forwarding to the `web` |
...
Detailed instructions (including `deploy.py` usage, classic Docker workflow, db-only mode, troubleshooting) live in **`../uk/DEPLOYMENT.md` (UA)** and **`DEPLOYMENT.md` (EN)**. Open that file after cloning to follow the recommended flow.
...
| `GET`  | `/products/`               | List + filter + paginate products (`search`, `min_price`, `ordering`) |
